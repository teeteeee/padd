...
const clientId =
  "216670948396-tp640nlrbid64f3p50jvvafiuk0l18j2.apps.googleusercontent.com";

const Login = () => {
  const [usedeet, setUserdeet] = useState(null);
  const { setUser } = useUserContext();

  const responseGoogleSuccess = (response) => {
    console.log("Login Success", response);
    const details = jwtDecode(response.credential);
    console.log("Decoded details:", details); // Log decoded details to inspect
    setUser(details); // Assuming details contains properties including "name"
    setUserdeet(details);

    // Store tokens in local storage
    localStorage.setItem("access_token", response.accessToken);
    localStorage.setItem("id_token", response.tokenId);
    localStorage.setItem("token_expiration", details.exp * 1000);
  };

  useEffect(() => {
    console.log("user state:", usedeet); // Log the user state to inspect
  }, [usedeet]);
